...
def select(acc_data):  #  target（acc_data）
    '''
    提供用户查询接口。
    :return: null
    '''
...
@auth
def s_select(acc_data):
    '''
    提供用户查询接口。
    :return: null
    '''
    user = input("请输入要查询的管理员>>:")
    #print(settings.DATABASE["path"])
    if os.path.isfile(settings.DATABASE["path"]+user+'.json'):
        #os.path.isfile(BASE_DIR + '\\accounts\\' + user + '.json')
        for i in getuserinfo(user):
...
@auth
def s_remove(acc_data):
    '''
    用户以及日志信息删除。
    :return:
    '''

    username = input("请输入你要注销的管理员：").strip()
    if os.path.isfile(settings.DATABASE["path"]+username+'.json'):
        os.remove(settings.DATABASE["path"]+username+'.json')
        print("注销管理员%s成功" %username)
        writeatmlog("注销管理员%s成功" %username)
    else:
        print("你输入的管理员不存在")
        writeatmlog("你输入的管理员%s不存在" %username)
...
def s_frozen(acc_data):
    ...
    if act == "1":
        fromuser["frozon"] = True
        log = "冻结%s管理员成功" %username
    elif act == "2":
        fromuser["frozon"] = False
        log = "解冻%s管理员成功" %username
    else:
        print("输入有误")
        return "fail"
    changeuserinfo(username,fromuser)
